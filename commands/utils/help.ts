...
module.exports = {
  ...
  async execute(message: Message, args: string[] | string, client: ClientExtensionInterface) {
    const prefix:string = client.PREFIX as string
    ...
    if(args.length == 0){
      ...
      .setTimestamp()
      .setFooter(`- ${botName} loves you <3`)
      .setColor(client.ClientFunction.generateColor())
      const commandGroups:Collection<string, Collection<string, Command>> = client.MessageCommandGroups
