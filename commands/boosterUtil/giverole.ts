...
module.exports = {
  ...
  async execute(message: Message, args: string[] | string, client: ClientExtensionInterface){
    ...
    if(!message.mentions.members?.first()) return message.reply("Please mention the member you want to give your role to.")
    ...
    const roleLimit = await client.ClientDatabase.guildData.find({guildID: message.guild?.id})
    if(roleLimit.length == 0 || !roleLimit[0].boostRoleLimit) return message.reply("It seems like this server is not configured to for this command to work. Please try again or contact an admin!")
    const roleMemberSize:number = message.guild.roles.cache.get(customroleid)?.members.size as number
    if(roleMemberSize > roleLimit[0].boostRoleLimit) return message.reply("You have reached the maximum allowed members for this custom role!")
    await message.mentions.members?.first()?.roles.add(role).catch((err) => {
      return message.reply("It seems like I am unable to give them your role! Please try again or contact an admin.")
    })
    return message.reply("I have added the role" + "`" +`${role.name}` + "`" + " to " + "`" + message.mentions.members.first()?.nickname + "`")
