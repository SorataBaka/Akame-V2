...
module.exports = {
  ...
  async execute(message: Message, args: string[] | string, client: ClientExtensionInterface){
    ...
    const tokenQuery = await client.ClientDatabase.boostertoken.find({guildid: message.guild?.id, memberid: message.member?.id})
    if(tokenQuery.length == 0) return message.reply("Either you are not a booster of this server or your token doesn't exist.")
    const token = tokenQuery[0].token
    const tokenEmbed = new MessageEmbed()
      .setThumbnail(client.user?.avatarURL() as string)
      .setAuthor(`Thank you so much for boosting the server ${message.member?.displayName}!!!`)
