...
module.exports = {
  ...
  async execute(message: Message, args: string[] | string, client: ClientExtensionInterface){
    ...
    await message.mentions.members?.first()?.roles.remove(role).catch((err) => {
      return message.reply("It seems like I am unable to remove your role! Please try again or contact an admin.")
    })
    return message.reply("I have removed the role" + "`" +`${role.name}` + "`" + " from " + "`" + message.mentions.members.first()?.nickname + "`")
